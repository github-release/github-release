...
// findAssetRegexp returns the asset if an asset with regexp matching name can be found in assets,
// otherwise returns nil.
func findAssetRegexp(assets []Asset, pattern string) *Asset {
	p := regexp.MustCompile(pattern)

	for _, asset := range assets {
		if p.MatchString(asset.Name) {
			return &asset
		}
	}
	return nil
